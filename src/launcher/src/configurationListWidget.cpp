...
static QByteArray GetPic0(const Configuration& info) {
	if (info.basedir.isEmpty()) {
		return {};
	}
	return GameContent::ReadFile(info.basedir, QStringLiteral("sce_sys/pic0.png"));
}
...
void ConfigurationListWidget::ScanGameDirectory() {
	...
	const auto    add_game = [this, &found_games](const QString& base, const QString& game_path,
		...
		const auto metadata = GetGameMetadata(
		    GameContent::ReadFile(base, QStringLiteral("sce_sys/param.json")), fallback);
	...
	const auto scan_archives = [&add_game, &eboot_name](const QDir& directory, const QDir& root) {
		const auto archives = directory.entryInfoList({QStringLiteral("*.zar")},
		                                              QDir::Files | QDir::NoSymLinks, QDir::Name);
		for (const auto& archive: archives) {
			const auto base = archive.absoluteFilePath();
			if (GameContent::FileExists(base, eboot_name)) {
				const auto game_path = QDir::cleanPath(base);
				add_game(base, game_path, root.relativeFilePath(base), archive.completeBaseName(),
				         true);
			}
		}
	};
...
void ConfigurationListWidget::SelectItem(QTreeWidgetItem* witem) {
	...
	m_ui->cfgs_list->SetBackgroundImage(item->GetInfo().basedir, GetPic0(item->GetInfo()));
